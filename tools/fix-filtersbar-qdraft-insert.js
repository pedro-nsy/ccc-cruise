...
const BAK = FILE + ".bak-9.1-qDraft-insert-final";
if (!fs.existsSync(BAK)) fs.copyFileSync(FILE, BAK);
let s = fs.readFileSync(FILE, "utf8");

// Ensure "use client" is first (safe even if already correct)
s = s.replace(/^\uFEFF?/, "");
s = s.replace(/^\s*"use client";\s*/m, "");
s = `"use client";\n` + s;

// Ensure react import has useState/useEffect
...
// If qDraft not declared, insert it right after function opening brace
...
// Make Clear reset the draft exactly once
s = s.replace(
  /onClick=\{\s*\(\)\s*=>\s*\{\s*setQ\(""\);\s*setType\(""\);\s*setStatus\(""\);\s*setUsed\(""\);\s*setQDraft\(""\);\s*setQDraft\(""\);\s*\}\s*\}/,
  'onClick={() => { setQ(""); setType(""); setStatus(""); setUsed(""); setQDraft(""); }}'
);

fs.writeFileSync(FILE, s, "utf8");
console.log("✓ FiltersBar: qDraft inserted + synced; Clear resets draft. Backup:", BAK);
