...
if (!fs.existsSync(FILE)) console.warn("Note: creating new route.ts at", FILE);
...
export async function GET(req: Request) {
  ...
  // Global stats from summary view + caps
  const { data: sums, error: sumErr } = await supabase
    .from("promo_codes_summary")
    .select("type, created, consumed, active_open");
  if (sumErr) return NextResponse.json({ ok:false, error:"STATS_SUMMARY_FAILED", detail:sumErr.message }, { status: 500 });
  ...
  (capsRows||[]).forEach(r => { if (r?.type) caps[r.type] = r.cap == null ? null : Number(r.cap); });

  return NextResponse.json({ ok:true, items: items || [], total: total || 0, stats: { created, consumed, caps, active_open: activeOpen } });
}
`;
fs.writeFileSync(FILE, content, "utf8");
console.log("✓ Wrote API route with stats.active_open. Backup:", path.basename(BAK));
