...
export async function GET(req: Request) {
  ...
  const { data: createdRows, error: cErr } = await supabase
    ...
    .order("type");
  if (cErr) return NextResponse.json({ ok:false, error:"STATS_CREATED_FAILED", detail:cErr.message }, { status: 500 });
  ...
  const { data: consumedRows, error: uErr } = await supabase
    ...
    .eq("status", "consumed")
    .order("type");
  if (uErr) return NextResponse.json({ ok:false, error:"STATS_USED_FAILED", detail:uErr.message }, { status: 500 });
  ...
  return NextResponse.json({ ok:true, items: items || [], total: total || 0, stats: { created, consumed, caps } });
}
`;
fs.writeFileSync(FILE, content, "utf8");
console.log("✓ Wrote API GET /api/admin/promos with global stats to the correct path. Backup:", BAK);
