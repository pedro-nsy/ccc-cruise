...
export default function StatsStrip({ stats }: { stats: any | null }) {
  ...
  const activeOpen: StatObj = {
    early_bird: Number(stats?.active_open?.early_bird ?? NaN),
    artist:     Number(stats?.active_open?.artist     ?? NaN),
    staff:      Number(stats?.active_open?.staff      ?? NaN),
  };
  const caps: CapsObj = {
  ...
  // Remaining uses active_open if present; otherwise falls back to consumed
  const remEB = typeof caps.early_bird === "number"
    ? Math.max(0, caps.early_bird - (Number.isNaN(activeOpen.early_bird) ? consumed.early_bird : activeOpen.early_bird))
    : null;
  const remArtist = typeof caps.artist === "number"
    ? Math.max(0, caps.artist - (Number.isNaN(activeOpen.artist) ? consumed.artist : activeOpen.artist))
    : null;
...
console.log("✓ StatsStrip now uses stats.active_open for Remaining. Backup:", path.basename(BAK));
