...
const BAK  = FILE + ".bak-step4b";

function backupWrite(file, content, tagPath){
  const dir = path.dirname(file);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  if (fs.existsSync(file) && !fs.existsSync(tagPath)) fs.copyFileSync(file, tagPath);
  fs.writeFileSync(file, content, "utf8");
  console.log("✓ wrote", file);
  console.log("  backup:", fs.existsSync(tagPath) ? tagPath : "(none)");
}
