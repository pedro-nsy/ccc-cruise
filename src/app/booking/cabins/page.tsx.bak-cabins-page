...
import { getEbRemaining, getArtistRemaining } from "../../../lib/config/caps";
...
function RoleChips({ category }: { category: "INTERIOR"|"OCEAN"|"BALCONY" }) {
  try {
    const eb = getEbRemaining(category);
    const ar = getArtistRemaining(category);
    return (
...
{chosen && <RoleChips category={chosen as Category} />}
