...
export default function Page() {
  ...
  const invalidFullWithDeposit = method === "deposit_mx" && payMode === "FULL";
    ...
    Hold active — reserved until <strong>{deadline}</strong> (business days).
      ...
      <section className="rounded-2xl border bg-white p-6 space-y-4">
        ...
        <div className="space-y-3">
          ...
          {method === "cash_mxn" && deadlineStr ? (<div className="rounded-xl border bg-neutral-50 p-3 text-sm text-neutral-700">Please complete payment by <strong>{deadlineStr}</strong>.</div>) : null}</label>
          ...
          {method === "cash_usd" && deadlineStr ? (<div className="rounded-xl border bg-neutral-50 p-3 text-sm text-neutral-700">Please complete payment by <strong>{deadlineStr}</strong>.</div>) : null}</label>

          {/* Cash deposit at bank — MXN (deposit only) */}
          <label className="flex flex-col gap-2 rounded-xl border p-4 hover:border-neutral-300 cursor-pointer">
            <div className="flex items-center gap-3">
              <input type="radio" name="method" value="deposit_mx" checked={method === "deposit_mx"} onChange={() => setMethod("deposit_mx")} />
              <div className="font-medium">Cash deposit at bank — Mexico (MXN)</div>
            </div>
            <div className="text-sm text-neutral-600">No processing fee. Deposit only.</div>
            {method === "deposit_mx" && (
              payMode === "DEPOSIT" ? (
                <div className="rounded-xl border bg-neutral-50 p-3 text-sm text-neutral-700">
                  Make a cash deposit at a bank branch into our account. We’ll provide account details after you confirm.
                  Email the deposit slip with your booking ID.
                </div>
              ) : (
                <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
                  Cash deposit at bank is only available when paying the deposit. To pay in full, choose another method.
                </div>
              )
            )}
          {method === "bank_deposit_mx" && deadlineStr ? (<div className="rounded-xl border bg-neutral-50 p-3 text-sm text-neutral-700">Please complete payment by <strong>{deadlineStr}</strong>.</div>) : null}</label>

          {/* US/Canada by contact */}
          <label className="flex flex-col gap-2 rounded-xl border p-4 hover:border-neutral-300 cursor-pointer">
            <div className="flex items-center gap-3">
              <input type="radio" name="method" value="us_canada" checked={method === "us_canada"} onChange={() => setMethod("us_canada")} />
              <div className="font-medium">Pay in US or Canada (cash or transfer)</div>
            </div>
            <div className="text-sm text-neutral-600">No processing fee.</div>
            {method === "us_canada" && (
              <div className="rounded-xl border bg-neutral-50 p-3 text-sm text-neutral-700">
                Contact us at payments@intertravel.mx with your booking ID and location. We’ll reply with instructions.
              </div>
            )}
          {method === "us_canada" && deadlineStr ? (<div className="rounded-xl border bg-neutral-50 p-3 text-sm text-neutral-700">If you choose this method, please contact us before <strong>{deadlineStr}</strong>.</div>) : null}</label>
        </div>
      </section>
